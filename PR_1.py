        ...
        try:
            with open("books.txt", "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    parts = line.split("|")
                    if len(parts) == 3:
                        title, author, status_str = parts
                        is_available = (status_str == "доступна")
                        self._books.append(Book(title, author, is_available))
        ...
        try:
            with open("users.txt", "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if len(parts) >= 2:
                        name = parts[0]
                        borrowed_books = parts[1].split(",") if parts[1] else []
                        borrowed_books = [b for b in borrowed_books if b]
                        self._users.append(User(name, borrowed_books))
        except FileNotFoundError:
            pass
        ...
        try:
            with open("books.txt", "w", encoding="utf-8") as f:
                for book in self._books:
                    ...
                    line = f"{book.get_title()}|{book.get_author()}|{status}"
                    f.write(line + "\n")
        
            with open("users.txt", "w", encoding="utf-8") as f:
                for user in self._users:
                    borrowed = ",".join(user.get_borrowed_books())
                    line = f"{user.get_name()}|{borrowed}"
                    f.write(line + "\n")
...
while True:
    ...
    if choice == "1":
        ...
        if library.login(name, "librarian"):
            while True:
                print(f"\nБиблиотекарь: {name}")
                options = library.get_current_user().get_menu_options()
        ...
        if library.login(name, "user"):
            ...
            print("Пользователь не найден. Сначала зарегистрируйтесь через меню библиотекаря.")
